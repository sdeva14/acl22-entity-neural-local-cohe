...
def validate(model, evaluator, dataset_test, config, loss_func, is_test=False):
    ...
    for text_inputs, label_y, *remains in dataloader_test:
        ...
        with torch.no_grad():
            ...
            if config.gen_logs:
                ...
                batch_cp_data = model_outputs[4]
                batch_num_sents = model_outputs[5]

                batch_center_trans = model_outputs[6]
                ...
                center_trans_list = center_trans_list + batch_center_trans

                tid_list = tid_list + tid.flatten().tolist()
                label_list = label_list + label_y.flatten().tolist()

                if config.target_model.lower() == "cent_attn":
                    # cur_cp_list is the indexes of CPs, then need to find the ind and convert them to words
                    cp_valid_list = update_cp_seq(len_seq, len_sents, config, batch_cp_data, text_inputs, cp_valid_list)

                elif config.target_model.lower() == "cent_ctrans":
                    # cur_cp_list is already 2d string list, then we just need to merge 2d list repeatdely
                    cp_valid_list += batch_cp_data
    ...
    valid_itpt = [tid_list, label_list, adj_list, root_ds_list, seg_map_list, cp_valid_list, center_trans_list]

    return valid_loss, eval_measure, eval_best_val, valid_itpt
# end validate

def update_cp_seq(len_seq, len_sents, config, cur_cp_ind, text_inputs, cp_list):
...
#           # get loss
            ...
            if config.gen_logs:
                ...
                # cur_cp_list = cur_cp_list + model_outputs[4]  # when it is np list, 2d list of strings
                num_sents_list = num_sents_list + model_outputs[5]

                center_trans_list = center_trans_list + model_outputs[6]

                cur_cp_data = model_outputs[4]

                if config.target_model.lower() == "cent_attn":
                    # cur_cp_list is the indexes of CPs, then need to find the ind and convert them to words
                    cp_list = update_cp_seq(len_seq, len_sents, config, cur_cp_data, text_inputs, cp_list)
            ...
            if batch_cnt % ckpt_step == 0:  # manual epoch printing
                ...
                if eval_cur_valid >= best_eval_valid or dataset_valid is None:
                    ...
                    if config.gen_logs and (config.target_model.lower() == "cent_attn" or config.target_model.lower() == "cent_ctrans"):
                        ...
                        cp_list = cp_list + valid_itpt[5]

                        center_trans_list = center_trans_list + valid_itpt[6]
                    ...
                    if eval_best > final_eval_best: 
                        ...
                        if config.gen_logs:
                            ...
                            # log for structure
                            if config.target_model.lower() == "cent_attn" or config.target_model.lower() == "cent_ctrans":
                                stru_log_name = "log_stru_" + config.corpus_target.lower() + "_" + str(config.essay_prompt_id_train) + "_" + str(config.cur_fold) + ".log"
                                trans_log_name = "log_trans_" + config.corpus_target.lower() + "_" + str(config.essay_prompt_id_train) + "_" + str(config.cur_fold) + ".log"
                                ...
                                # center trans test
                                col_trans = ["tid", "label", "trans"]
                                with open(os.path.join(config.session_dir, trans_log_name), "w") as log_file:
                                    writer = csv.DictWriter(log_file, fieldnames=col_trans)
                                    for i in range(len(tid_list)):
                                        writer.writerow({'tid': tid_list[i], 'label': label_list[i], 'trans': center_trans_list[i]})
        ...
        if config.gen_logs and (config.target_model.lower() == "cent_attn" or config.target_model.lower() == "cent_ctrans"):
            ...
            with open(os.path.join(config.session_dir, cp_log_name), "w") as log_file:
                ...
                for i in range(len(cp_list)):
                    writer.writerow({'tid': tid_list[i], 'doc_cp_seq': cp_list[i]})

            print(wlekfjwlekjfewljlekwf)  # exit
