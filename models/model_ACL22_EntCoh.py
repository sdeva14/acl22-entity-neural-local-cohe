...
class Model_ACL22_EntCoh(models.model_base.BaseModel):
    '''
    The model proposed in ACL22 paper, Entity-based Neural Local Coherence Modeling
    '''
    def __init__(self, config, corpus_target, embReader):
        ...
        #        
        self.document_weight = nn.Parameter(torch.zeros(self.base_encoder.encoder_out_size,1))
        nn.init.xavier_uniform_(self.document_weight)
...
#####################################################################################
        ...
        for sent_i in range(self.max_num_sents):
            ...
            selected, batch_cp_nps, np_all_repr, num_np_sents = \
                                     self.select_fwrd_centers_np(sent_i, batch_map_np, encoded_sent, \
                                                                 attn_diag, batch_cp_nps, avg_sents_repr, np_all_repr, num_np_sents, fwrd_repr)
    ...
    def select_fwrd_centers_np(self, sent_i, batch_map_np, encoded_sent, attn_diag, batch_cp_nps, avg_sents_repr, np_all_repr, num_np_sents, fwrd_repr):
        ...
        for cur_batch in range(batch_size):
            ...
            if cur_num_np_sent < 1:
                # exception: there is no entity focus, as we described in the paper, e.g.,) Ellipsis
    ...
    def centering_attn(self, text_inputs, mask_input, len_sents, num_sents, tid):
        '''
        Originated from the EMNLP20 model, part of model is redundant but maintained for logging and comparing
        '''
    ...
    def get_np_pair_repr(self, np_all_repr, num_np_sents, len_sents, num_sents, sent_mask, tid):
        ...
        # semantic consistency in local focus
        sim_repr = torch.zeros(batch_size, np_pair_avg.size(1)-1)
        sim_repr = utils.cast_type(sim_repr, FLOAT, self.use_gpu)

        sim_const = torch.zeros(batch_size)
        sim_const = utils.cast_type(sim_const, FLOAT, self.use_gpu)
        for cur_batch in range(batch_size):
            cur_pair_num = num_sents[cur_batch]-1
            dist_list = []
            for cur_ind in range(np_pair_avg.size(1)-1):
                if cur_ind > cur_pair_num:
                    break

                cur_pair = np_pair_avg[cur_batch, cur_ind]
                next_pair = np_pair_avg[cur_batch, cur_ind+1]

                cur_dist = self.sim_cosine_d0(cur_pair, next_pair)
                dist_list.append(float(cur_dist))

                sim_repr[cur_batch, cur_ind] = cur_dist

        return np_pair_avg, mask_np_pair_avg, sim_const, sim_repr
...
#####################################################################################
    ...
    def forward(self, text_inputs, mask_input, len_seq, len_sents, tid, len_para=None, list_rels=None, mode=""):
        ...
        #### Stage 1: Sentence repr and discourse segments parser
        ...
        #### Stage 2-1: Adjacent NP Focus Pair
        np_pair_avg, mask_np_pair_avg, sim_const, local_focus_sim = self.get_np_pair_repr(np_all_repr, num_np_sents, len_sents, num_sents, sent_mask, tid)
        #### Stage 2-2: NP with dconv
        np_pair_avg = self.dconv2(np_pair_avg)
        np_pair_avg = self.leak_relu(np_pair_avg)

        #### Stage 3: NP with tree transformer
        encoded_np_pair, _ = self.tree_encoder2(np_pair_avg, mask_np_pair_avg)

        # Stage 4:document attention
        document_weight = self.document_weight.expand(encoded_np_pair.shape[0], encoded_np_pair.shape[2], 1)
        attn_weight = torch.bmm(encoded_np_pair, document_weight).squeeze(2)
        attn_weight = self.tanh(attn_weight)
        attn_weight = masked_softmax(attn_weight, mask_np_pair_avg)
        attn_vec = torch.bmm(encoded_np_pair.transpose(1, 2), attn_weight.unsqueeze(2))
        attn_np_pair = attn_vec.squeeze(2)

        ## Stage 5: Combine entity-modeling and context vector
        ilc_vec_sent = torch.div(torch.sum(sent_repr, dim=1), num_sents.unsqueeze(1))  # (batch_size, rnn_cell_size)
        ilc_vec = torch.cat((ilc_vec_sent, attn_np_pair), dim=1)
