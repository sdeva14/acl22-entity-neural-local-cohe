...
class GroupAttention(nn.Module):
    def __init__(self, d_model, dropout=0.8):
        ...
        # self.d_model = 256.  # fixed in the original implemenation?
        self.d_model = d_model  # fixed in the original implemenation?
        self.linear_key = nn.Linear(d_model, d_model)
        self.linear_query = nn.Linear(d_model, d_model)
        #self.linear_output = nn.Linear(d_model, d_model)
        
        self.norm = LayerNorm(d_model)
        # self.norm = BertLayerNorm(d_model)
    ...
    def forward(self, context, eos_mask, prior):
        ...
        # only allow to see the past items
        ...
        scores = scores.masked_fill(mask == 0, -1e9)  # to prevent zero division by low value
