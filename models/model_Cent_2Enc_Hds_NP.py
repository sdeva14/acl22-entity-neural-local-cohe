...
class Coh_Model_Cent_2Enc_Hds_NP(models.model_base.BaseModel):
    def __init__(self, config, corpus_target, embReader):
        ...
        # vocab = set(test_sentence)  # it might not need if we directly assign index to transitions
        ...
        self.dconv_nb_1 = fairseq.LightweightConv1dTBC(input_size=self.max_num_sents-1, 
                            kernel_size=5,
                            padding_l=2,
                            num_heads=4,
                            weight_dropout=0.1,
        ...
        self.dconv_nb_2 = fairseq.LightweightConv1dTBC(input_size=self.max_num_sents-1, 
                            kernel_size=3,
                            padding_l=2,
                            num_heads=1,
                            weight_dropout=0.1,
...
#####################################################################################
    ...
    def get_fwrd_centers_np_enc2(self, text_inputs, mask_input, len_sents, num_sents, tid):
        ...
        for sent_i in range(self.max_num_sents):
            ...
            # (what if there are NP fewer than top-k?)
            ...
            # simple idea is to assign zero vector
            for cur_batch in range(batch_size):
    ...
    def get_fwrd_centers_np(self, text_inputs, mask_input, len_sents, num_sents, tid):
        ...
        # return avg_sents_repr, fwrd_repr, np_avg_repr, batch_cp_ind
        return avg_sents_repr, fwrd_repr, np_avg_repr, batch_cp_nps
    # end def get_fwrd_centers
    ...
    def centering_attn(self, text_inputs, mask_input, len_sents, num_sents, tid):
        ...
        # avg_sents_repr, fwrd_repr, np_avg_repr, batch_cp_nps = self.get_fwrd_centers_np(text_inputs, mask_input, len_sents, num_sents, tid)
        ...
        return  adj_mat, avg_sents_repr, batch_adj_list, batch_root_list, batch_segMap, batch_cp_nps, batch_center_trans, trans_inputs, trans_mask, fwrd_repr, np_avg_repr
...
#####################################################################################
    ...
    def forward(self, text_inputs, mask_input, len_seq, len_sents, tid, len_para=None, list_rels=None, mode=""):
        ...
        adj_mat, sent_repr, batch_adj_list, batch_root_list, batch_segMap, batch_cp_nps, batch_center_trans, \
         trans_inputs, trans_mask, fwrd_repr, np_avg_repr = self.centering_attn(text_inputs, mask_input, len_sents, num_sents, tid)
        ...
        context_weight = self.context_weight_2.expand(encoded_sents.shape[0], encoded_sents.shape[2], 1)
        attn_weight = torch.bmm(encoded_sents, context_weight).squeeze(2)
        attn_weight = self.tanh(attn_weight)
        attn_weight = masked_softmax(attn_weight, trans_mask)
        attn_vec = torch.bmm(encoded_sents.transpose(1, 2), attn_weight.unsqueeze(2))
        attn_stru_repr = attn_vec.squeeze(2)

        
        ilc_vec = attn_stru_repr
