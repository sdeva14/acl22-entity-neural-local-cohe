...
from transformers import AutoModel  # to load model weights to huggingface style easily

class Encoder_SpanBERT(nn.Module):
    ...
    def __init__(self, config, x_embed):
        ...
        path_model_weights = ""  # need to be stored in local storage, then load
        model = AutoModel.from_pretrained(path_model_weights)
