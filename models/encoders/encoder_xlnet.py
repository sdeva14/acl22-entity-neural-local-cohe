...
class Encoder_XLNet(nn.Module):
    ...
    def forward(self, text_inputs, mask_input, len_seq, mode=""):
        ...
        with torch.no_grad():
            ...
            encoded = model_output.last_hidden_state * mask_input.unsqueeze(2)

            encoder_out.append(encoded)

            if self.output_attentions:
                # attn = model_output[1][-1] # only consider mh attentions from the last layer (batch, mh, item, item)
                attn = model_output.attentions[-1]  # the last layer of mh attentions  
    ...
    # encoded_sent = cur_encoded.last_hidden_state  # encoded output for the current sent
    # attn_last_layer = cur_encoded.attentions[-1]  # the last layer of attention for the current sent
    # attn_sent = torch.div(torch.sum(attn_last_layer, dim=1), attn_last_layer.size(1))  # average for all head
