...
from models.model_EMNLP19_Unified import Model_EMNLP19_Unified
...
from models.model_Cent_Hds_NP import Coh_Model_Cent_Hds_NP
from models.model_Cent_2Enc_Hds_NP import Coh_Model_Cent_2Enc_Hds_NP

from models.model_2Enc_Avg import Coh_Model_2Enc_Avg
from models.model_ACL22_EntCoh import Model_ACL22_EntCoh

from models.model_DIS_Avg_Inp import Coh_Model_DIS_Avg_Inp
...
def get_model_target(config, corpus_target, embReader):
    ...
    if config.target_model.lower().startswith("emnlp18"):
        logger.info("Model: EMNLP18 Coherence")
        ...
        logger.info("Model: CoNLL17 Essay Scoring")
        model = Model_CoNLL17_Essay(config=config, corpus_target=corpus_target, embReader=embReader)
    if config.target_model.lower().startswith("emnlp19_unified"):
        logger.info("Model: EMNLP19 Unified")
        model = Model_EMNLP19_Unified(config=config, corpus_target=corpus_target, embReader=embReader)
    ...
    elif config.target_model.lower() == "cent_hds_np":
        logger.info("Model: Centering Structure NP Ver")
        model = Coh_Model_Cent_Hds_NP(config=config, corpus_target=corpus_target, embReader=embReader)
    elif config.target_model.lower() == "cent_2enc_hds_np":
        logger.info("Model: Centering Structure NP Ver")
        model = Coh_Model_Cent_2Enc_Hds_NP(config=config, corpus_target=corpus_target, embReader=embReader)

    elif config.target_model.lower() == "avg_2enc":
        logger.info("Model: Averaging Adjacent Transformer with Interpolated")
        model = Coh_Model_2Enc_Avg(config=config, corpus_target=corpus_target, embReader=embReader)
    elif config.target_model.lower() == "acl22_entcoh":
        logger.info("Model: ACL22 Entity-based Centering")        
        model = Model_ACL22_EntCoh(config=config, corpus_target=corpus_target, embReader=embReader)

    elif config.target_model.lower() == "dis_avg_inp":   
        logger.info("Model: Averged ")
        model = Coh_Model_DIS_Avg_Inp(config=config, corpus_target=corpus_target, embReader=embReader)
...
def exp_model(config):
    ...
    #### read NP information from the file generated in the pre-processing
    # np_file_path = os.path.join(config.data_dir, "nounp_old")  # stanza library 1.0.1 ver
    config.map_np_sbw_total, config.max_num_np = corpus_target.get_np_info(config)

    # config.map_avg_sim = corpus_target.map_avg_sim  ## (emnlp21 test)
...
if __name__=='__main__':
    ts = time.time()
    start_date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
    ...
    if config.corpus_target.lower() == "toefl":
        ...
        cur_domain_train = config.corpus_target.lower()
        cur_domain_test = config.corpus_target.lower()
    ...
    if config.cur_fold > -1:  # test for specific fold
        for cur_attempt in range(target_attempts):
    ...
    logger.info("Start date {}".format(end_date))